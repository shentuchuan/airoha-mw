...
#define MQTTD_RATE_TICK_OFFSET (6)
...
typedef struct MQTTD_CTRL_S
{
    ...
    C8_T topic_prefix[MQTTD_MAX_TOPIC_PREFIX_SIZE]; /*{manufacturer}/sw/{sn}*/
...
#define MQTT_CHECK_COND(__shift__, __op__, __size__) \
    do                                               \
    {                                                \
...
static void _mqttd_ctrl_init(MQTTD_CTRL_T *ptr_mqttd, ip_addr_t *server_ip)
{
    ...
    // 分配内存
    ptr_mqttd->rx = (UI32_T *)mqtt_malloc(PLAT_MAX_PORT_NUM * sizeof(UI32_T));
    ptr_mqttd->tx = (UI32_T *)mqtt_malloc(PLAT_MAX_PORT_NUM * sizeof(UI32_T));
    ptr_mqttd->rx_rate = (UI32_T *)mqtt_malloc(PLAT_MAX_PORT_NUM * sizeof(UI32_T));
    ptr_mqttd->tx_rate = (UI32_T *)mqtt_malloc(PLAT_MAX_PORT_NUM * sizeof(UI32_T));

    // 初始化为0
    if (ptr_mqttd->rx)
    {
        osapi_memset(ptr_mqttd->rx, 0, PLAT_MAX_PORT_NUM * sizeof(UI32_T));
    }
    if (ptr_mqttd->tx)
    {
        osapi_memset(ptr_mqttd->tx, 0, PLAT_MAX_PORT_NUM * sizeof(UI32_T));
    }
    if (ptr_mqttd->rx_rate)
    {
        osapi_memset(ptr_mqttd->rx_rate, 0, PLAT_MAX_PORT_NUM * sizeof(UI32_T));
    }
    if (ptr_mqttd->tx_rate)
    {
        osapi_memset(ptr_mqttd->tx_rate, 0, PLAT_MAX_PORT_NUM * sizeof(UI32_T));
    }
...
static void _mqttd_ctrl_free(MQTTD_CTRL_T *ptr_mqttd)
{
    ...
    if (ptr_mqttd->rx)
    {
        mqtt_free(ptr_mqttd->rx);
        ptr_mqttd->rx = NULL;
    }
    if (ptr_mqttd->tx)
    {
        mqtt_free(ptr_mqttd->tx);
        ptr_mqttd->tx = NULL;
    }
    if (ptr_mqttd->rx_rate)
    {
        mqtt_free(ptr_mqttd->rx_rate);
        ptr_mqttd->rx_rate = NULL;
    }
    if (ptr_mqttd->tx_rate)
    {
        mqtt_free(ptr_mqttd->tx_rate);
        ptr_mqttd->tx_rate = NULL;
    }
#if 0
...
static void _mqttd_calc_port_traffic(MQTTD_CTRL_T *ptr_mqttd)
{
    MW_ERROR_NO_T rc = MW_E_OK;
    ...
    /* rxbytes */
    rc = httpd_queue_getData(MIB_CNT, MIB_CNT_RX_OCTETS, DB_ALL_ENTRIES, &ptr_msg, &size, (void **)&ptr_port);
    if (MW_E_OK != rc)
    {
        return;
    }
    ...
    {
        UI64_T current_rx = ptr_port[i];
        UI32_T last_rx = ptr_mqttd->rx[i];
        UI32_T current_rx_low = (UI32_T)(current_rx & 0xFFFFFFFF);
        UI32_T current_rx_high = (UI32_T)((current_rx >> 32) & 0xFFFFFFFF);

        if (current_rx_low >= last_rx)
        {
            ptr_mqttd->rx_rate[i] = (current_rx_low - last_rx) * 2 / MQTTD_RATE_TICK_OFFSET;
        }
        else
        {
            // Handle 32-bit address rollover
            ptr_mqttd->rx_rate[i] = ((0xFFFFFFFF - last_rx) + current_rx_low + 1) * 2 / MQTTD_RATE_TICK_OFFSET;
        }
        // Update the last received bytes
        ptr_mqttd->rx[i] = current_rx_low;
    }
    mqtt_free(ptr_msg);

    /* txbytes */
    rc = httpd_queue_getData(MIB_CNT, MIB_CNT_TX_OCTETS, DB_ALL_ENTRIES, &ptr_msg, &size, (void **)&ptr_port);
    if (MW_E_OK != rc)
    {
        return;
    }
    for (i = 0; i < PLAT_MAX_PORT_NUM; i++)
    {
        ...
        UI64_T current_tx = ptr_port[i];
        UI32_T last_tx = ptr_mqttd->tx[i];
        UI32_T current_tx_low = (UI32_T)(current_tx & 0xFFFFFFFF);
        UI32_T current_tx_high = (UI32_T)((current_tx >> 32) & 0xFFFFFFFF);

        if (current_tx_low >= last_tx)
        {
            ptr_mqttd->tx_rate[i] = (current_tx_low - last_tx) * 2 / MQTTD_RATE_TICK_OFFSET;
        }
        else
        {
            // Handle 32-bit address rollover
            ptr_mqttd->tx_rate[i] = ((0xFFFFFFFF - last_tx) + current_tx_low + 1) * 2 / MQTTD_RATE_TICK_OFFSET;
        }

        // Update the last transmitted bytes
        ptr_mqttd->tx[i] = current_tx_low;
    }
    mqtt_free(ptr_msg);
...
static void _mqttd_publish_status(MQTTD_CTRL_T *ptr_mqttd)
{
    ...
    for (i = 1; i <= PLAT_MAX_PORT_NUM; i++)
    {
        ...
        rc = mqttd_tmr_queue_getData(PORT_OPER_INFO, DB_ALL_FIELDS, i, &ptr_db_msg, &db_size, &db_data);
        if (MW_E_OK != rc)
        {
        ...
        rc = mqttd_tmr_queue_getData(PORT_CFG_INFO, DB_ALL_FIELDS, i, &ptr_db_msg, &db_size, &db_data);
        if (MW_E_OK != rc)
        {
        ...
        cJSON_AddStringToObject(json_port_entry, "name", port_name);
        ...
        cJSON_AddNumberToObject(json_port_entry, "txRate", ptr_mqttd->tx_rate[i - 1]);
        cJSON_AddNumberToObject(json_port_entry, "rxRate", ptr_mqttd->rx_rate[i - 1]);
...
static void _mqttd_publish_macs(MQTTD_CTRL_T *ptr_mqttd)
{
    ...
    rc = mqttd_tmr_queue_getData(STATIC_MAC_ENTRY, DB_ALL_FIELDS, DB_ALL_ENTRIES, &ptr_db_msg, &db_size, &db_data);
    if (MW_E_OK != rc)
    {
    ...
    */
    ...
    for (i = 1; i <= PLAT_MAX_PORT_NUM; i++)
    {
        ...
        BITMAP_VLAN_FOREACH(vlan_list, vid_idx)
        {
            UI16_T vlan_id = 0;
            // get vlan id
            rc = mqttd_tmr_queue_getData(VLAN_ENTRY, DB_ALL_FIELDS, vid_idx, &ptr_db_msg, &db_size, &db_data);
            if (MW_E_OK != rc)
            {
...
static void _mqttd_tmr(timehandle_t ptr_xTimer)
{
    ...
    {
        if (mqttd.ticknum % MQTTD_RATE_TICK_OFFSET == 0) /*every 3sec*/
        {
            _mqttd_calc_port_traffic(&mqttd);
        }
...
static MW_ERROR_NO_T _mqttd_publish_port_storm_ctrl(MQTTD_CTRL_T *ptr_mqttd, const DB_REQUEST_TYPE_T *req, const void *ptr_data)
{
    ...
    osapi_printf("publish port storm cfg: T/F/E =%u/%u/%u\n", req->t_idx, req->f_idx, req->e_idx);
...
static MW_ERROR_NO_T _mqttd_publish_port_vlan_setting(MQTTD_CTRL_T *ptr_mqttd, const DB_REQUEST_TYPE_T *req, const void *ptr_data)
{
    ...
    DB_MSG_T *ptr_vlan_entry_msg = NULL;
    ...
    u8_t port_start = 0;
    u8_t port_end = PLAT_MAX_PORT_NUM;
    if (0 != req->e_idx)
    {
        // 上报指定端口的vlan
        port_start = req->e_idx;
        port_end = req->e_idx + 1;
    }

    osapi_printf("publish port vlan: T/F/E =%u/%u/%u\n", req->t_idx, req->f_idx, req->e_idx);
    ...
    for (port_id = port_start; port_id < port_end; port_id++)
    {
        ...
        // 端口不存在vlan配置则查找下一个
        if ((VLAN_DEFAULT_VID == ptr_pvid_tbl[port_id]) && (0 == ptr_vlan_list_tbl[port_id]))
        {
            continue;
        }
        ...
        if (0 != ptr_vlan_list_tbl[port_id])
        {
            ...
            BITMAP_VLAN_FOREACH(ptr_vlan_list_tbl[port_id], j)
            {
                ...
                if (vlan_entry->vlan_id != 0)
                {
                    ...
                    {
                        vlan1_id[vlan1_cnt] = vlan_entry->vlan_id;
                        vlan1_cnt++;
                        MW_FREE(ptr_vlan_entry_msg);
                        ptr_vlan_entry_msg = NULL;
                        continue;
                    }
                    if (vlan_entry->tagged_member & (1 << port_id))
                    {
                        vlan1_id[vlan1_cnt] = vlan_entry->vlan_id;
                        vlan1_cnt++;
                    }
                    if (vlan_entry->untagged_member & (1 << port_id))
                    {
                        vlan2_id[vlan2_cnt] = vlan_entry->vlan_id;
                        vlan2_cnt++;
                    }
                }
                MW_FREE(ptr_vlan_entry_msg);
        ...
        else if (port_vlan_type == MQTTD_PORT_VLAN_TRUNK)
        {
            ...
            {
                cJSON_AddItemToArray(pv, cJSON_CreateNumber(vlan1_id[j]));
            }
        ...
        else
        {
            ...
            for (j = 0; j < vlan1_cnt; j++)
            {
                cJSON_AddItemToArray(tv, cJSON_CreateNumber(vlan1_id[j]));
            }
            ...
            {
                cJSON_AddItemToArray(utv, cJSON_CreateNumber(vlan2_id[j]));
            }
    ...
    cJSON_AddStringToObject(root, "type", "config");
    cJSON_AddItemToObject(root, "data", data);
    cJSON_AddItemToObject(data, "vlan_setting", vlan_setting);
    vlan_setting = NULL;
...
static MW_ERROR_NO_T _mqttd_publish_loop_prevention(MQTTD_CTRL_T *ptr_mqttd, const DB_REQUEST_TYPE_T *req, const void *ptr_data)
{
    MW_ERROR_NO_T rc;
    DB_MSG_T *ptr_msg = NULL;
    ...
    rc = mqttd_queue_getData(LOOP_PREVEN_INFO, DB_ALL_FIELDS, DB_ALL_ENTRIES, &ptr_msg, &size, (void **)&ptr_lp);
    if (MW_E_OK != rc)
    {
    ...
    cJSON_AddStringToObject(root, "type", "config");
    cJSON_AddItemToObject(root, "data", data);
    cJSON_AddItemToObject(data, "loop_guard", cJSON_CreateBool(ptr_lp->enable));
...
static MW_ERROR_NO_T _mqttd_publish_portcfg(MQTTD_CTRL_T *ptr_mqttd, const DB_REQUEST_TYPE_T *req, const void *ptr_data)
{
    ...
    UI16_T db_size = 0;
    void *db_data = NULL;
    DB_PORT_CFG_INFO_T ptr_port_cfg_info;
    ...
    if ((PORT_STORM_BC_RATE <= req->f_idx) && (PORT_STORM_UC_CFG >= req->f_idx))
    {
        return _mqttd_publish_port_storm_ctrl(ptr_mqttd, req, ptr_data);
    }

    if (req->f_idx != PORT_ADMIN_STATUS && req->f_idx != DB_ALL_FIELDS)
        return MW_E_OK;
        
	memset(&ptr_port_cfg_info, 0, sizeof(DB_PORT_CFG_INFO_T));
    ...
    memcpy(&ptr_port_cfg_info, db_data, sizeof(DB_PORT_CFG_INFO_T));
    mqtt_free(db_msg);
    ...
    char port_name[10];
    snprintf(port_name, sizeof(port_name), "port%d", req->e_idx);
    cJSON_AddStringToObject(port_setting_entry, "n", port_name);
    cJSON_AddNumberToObject(port_setting_entry, "en", ptr_port_cfg_info.admin_status > 0 ? 1 : 0);
    /*an: 0
    10M:1
    100M:2
    1000M:3*/
    cJSON_AddNumberToObject(port_setting_entry, "sp", ptr_port_cfg_info.admin_speed);

    /*Duplex
    half: 3
    full: 4*/
    if (ptr_port_cfg_info.admin_speed == 0)
        cJSON_AddNumberToObject(port_setting_entry, "du", 0);
    else if (ptr_port_cfg_info.admin_duplex == 0)
        cJSON_AddNumberToObject(port_setting_entry, "du", 3); /*half*/
    else
        cJSON_AddNumberToObject(port_setting_entry, "du", 4); /*full*/
...
static MW_ERROR_NO_T _mqttd_publish_vlan_member(MQTTD_CTRL_T *ptr_mqttd, const DB_REQUEST_TYPE_T *req, const void *ptr_data)
{
    ...
    if (req->e_idx != 0)
    {
        ...
        {
            cJSON_AddItemToArray(vlan_member, cJSON_CreateNumber(vlan_entry->vlan_id));
            MW_FREE(db_msg);
        }
    }
    else
...
static MW_ERROR_NO_T _mqttd_publish_jumbo_frame(MQTTD_CTRL_T *ptr_mqttd, const DB_REQUEST_TYPE_T *req, const void *ptr_data)
{
    ...
    u16_t db_size = 0;
    void *db_data = NULL;
    osapi_printf("publish jumbo frame: T/F/E =%u/%u/%u\n", req->t_idx, req->f_idx, req->e_idx);
        ...
        if (M_UPDATE == (method & M_UPDATE))
        {
            ...
            while (count > 0)
            {
                ...
                case LOOP_PREVEN_INFO:
                    (void)_mqttd_publish_loop_prevention(ptr_mqttd, &req, ptr_data);
                    break;
...
static MW_ERROR_NO_T _mqttd_handle_setconfig_loop_prevention(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    MW_ERROR_NO_T rc = MW_E_OK;
    UI8_T en = (UI8_T)data_obj->valueint;
    mqttd_debug("Set loop prevention state to ===%s == %s == %d == %f ====\n", data_obj->string, data_obj->valuestring, data_obj->valueint, data_obj->valuedouble);
    mqttd_debug("Set loop prevention state to %d\n", en);
    /* Request loop prevention state to DB */
    rc = mqttd_queue_setData(M_UPDATE, LOOP_PREVEN_INFO, LOOP_PREVEN_ENABLE, DB_ALL_ENTRIES, &en, sizeof(UI8_T));
    if (MW_E_OK != rc)
    {
        return rc;
    }
        ...
        if (cJSON_IsObject(port_cfg_obj))
        {
            ...
            cJSON_ArrayForEach(port_id, port_obj)
            {
                if (cJSON_IsNumber(port_id) && port_id->valueint <= PLAT_MAX_PORT_NUM)
                {
                    ...
                    cJSON *duplex_obj = cJSON_GetObjectItemCaseSensitive(port_cfg_obj, "du");
                    if (duplex_obj)
                    {
                    ...
                    // osapi_printf("set port[%d] setting: in speed:%d, in duplex:%d, out duplex:%d\n", port_id_value, speed_obj->valueint, duplex_obj->valueint, port_cfg_info.admin_duplex);
                    cJSON *fc_obj = cJSON_GetObjectItemCaseSensitive(port_cfg_obj, "fc");
                    if (fc_obj)
                    {
                    ...
                    cJSON *eee_obj = cJSON_GetObjectItemCaseSensitive(port_cfg_obj, "EEE");
                    if (eee_obj)
                    {
...
static MW_ERROR_NO_T _mqttd_handle_setconfig_port_mirroring(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    ...
    ONE_DB_PORT_MIRROR_INFO_T port_mirror_info;
    DB_MSG_T *ptr_db_msg = NULL;
    DB_MSG_T *ptr_db_msg1 = NULL;
    u16_t db_size = 0;
    void *db_data = NULL;
    UI8_T port_mirror_id[PLAT_MAX_PORT_NUM];
        ...
        if (cJSON_IsObject(port_mirror_obj))
        {
            ...
            for (index = 0; index < src_port_size; index++)
            {
                ...
                if (cJSON_IsNumber(src_port_element) && cJSON_IsNumber(dir_element))
                {
                    ...
                    if (dir_int == 1)
                    {
                        port_mirror_info.src_in_port |= (1 << (port_int - 1));
                    }
                    else if (dir_int == 2)
                    {
                        port_mirror_info.src_eg_port |= (1 << (port_int - 1));
                    }
                    else if (dir_int == 3)
                    {
                        port_mirror_info.src_in_port |= (1 << (port_int - 1));
            ...
            // get port mirror id
            memset(&port_mirror_id[0], 0, sizeof(UI8_T) * PLAT_MAX_PORT_NUM);
            rc = mqttd_queue_getData(PORT_CFG_INFO, PORT_MIRROR_ID, DB_ALL_ENTRIES, &ptr_db_msg1, &db_size, (void **)&db_data);
            if (MW_E_OK != rc)
            {
                mqttd_debug("get org DB port_mirror_info failed(%d)\n", rc);
                break;
            }
            memcpy(&port_mirror_id[0], db_data, sizeof(UI8_T) * PLAT_MAX_PORT_NUM);
            mqtt_free(ptr_db_msg1);
            ...
            for (i = 0; i < PLAT_MAX_PORT_NUM; i++)
            {
                if (port_mirror_info.src_in_port & (1 << i) || port_mirror_info.src_eg_port & (1 << i))
                {
                    port_mirror_id[i] |= (1 << (session_id - 1));
                }
                else
                {
                    port_mirror_id[i] &= ~(1 << (session_id - 1));
            ...
            // update port mirror id
            rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_MIRROR_ID, DB_ALL_ENTRIES, &port_mirror_id[0], (sizeof(UI8_T) * PLAT_MAX_PORT_NUM));
            if (MW_E_OK != rc)
            {
                mqttd_debug("get org DB port_mirror_info failed(%d)\n", rc);
                break;
...
static MW_ERROR_NO_T _mqttd_handle_setconfig_static_mac(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    ...
    cJSON_ArrayForEach(static_mac_obj, data_obj)
    {
        if (cJSON_IsObject(static_mac_obj))
        {
            ...
            UI16_T port = port_obj->valueint;

            osapi_printf("conf_mac_addr: %02x:%02x:%02x:%02x:%02x:%02x, vid: %d, port: %d\n",
                         conf_mac_addr[0], conf_mac_addr[1], conf_mac_addr[2],
                         conf_mac_addr[3], conf_mac_addr[4], conf_mac_addr[5],
                         vid, port);
            ...
            if (strcmp(cmd_obj->valuestring, "add") == 0)
            {
                ...
                if (!found)
                {
                    ...
                    if (!find_blank)
                    {
                        mqttd_debug("static mac table is full!\n");
                        break;
                    }
                }
                else
                {
                    mqttd_debug("static_mac_info mac_addr(%s) vid(%d) already exists\n", mac_obj->valuestring, vid);
                    break;
                }
            }
            else if (strcmp(cmd_obj->valuestring, "del") == 0)
            {
                ...
                if (!found)
                {
                    mqttd_debug("static_mac_info mac_addr(%s) vid(%d) not found\n", mac_obj->valuestring, vid);
                    break;
                }
            }
            else if (strcmp(cmd_obj->valuestring, "modify") == 0)
            {
                int found = 0;
            ...
            if (cmd_obj)
            {
                if (strcmp(cmd_obj->valuestring, "add") == 0)
                {
                    ...
                    if (!entry_found)
                    {
                        for (i = 0; i < MAX_VLAN_ENTRY_NUM; i++)
                        {
                            if ((vlan_info.vlan_id[i] == 0) || (vlan_info.vlan_id[i] == i + 1))
                            { // Assuming 0 means empty entry
...
static MW_ERROR_NO_T _mqttd_vlan_mode_set(u8_t vlan_mode)
{
    ...
    if ((VLAN_PORT_ENABLE == vlan_mode && vlan_cfg->enable_port_b == VLAN_STATE_ENABLE) || (VLAN_1Q_ENABLE == vlan_mode && vlan_cfg->enable_8021q_b == VLAN_STATE_ENABLE) || (VLAN_MTU_ENABLE == vlan_mode && vlan_cfg->enable_mtu == VLAN_STATE_ENABLE))
    {
        MW_FREE(ptr_db_msg);
        return rc;
    }
...
static MW_ERROR_NO_T _mqttd_vlan_entry_add(VLAN_ENTRY_INFO_T *ptr_vlan_entry, u16_t *ptr_vidx)
{
    ...
    *ptr_vidx = 0;
    ...
    for (i = 1; i < MAX_VLAN_ENTRY_NUM + 1; i++)
    {
        ...
        {
            if ((vlan_entry->vlan_id == 0) || ((vlan_entry->vlan_id == i) && (vlan_entry->port_member == 0) && (vlan_entry->tagged_member == 0) && (vlan_entry->untagged_member == 0)))
            {
                empty_vlan_entry = i;
                vidx = i;
                // 保存vlan内容
    ...
    *ptr_vidx = 0;
    ...
    for (i = 0; i < MAX_VLAN_ENTRY_NUM; i++)
    {
        ...
        if (empty_entry_idx == 0)
        {
            ...
            {
                if (ptr_vlan_entry_tbl->port_member[i] == 0 && ptr_vlan_entry_tbl->tagged_member[i] == 0 && ptr_vlan_entry_tbl->untagged_member[i] == 0)
                {
                    empty_entry_idx = i + 1;
                }
            }
...
static MW_ERROR_NO_T _mqttd_set_storm_control(uint16_t port_idx, storm_ctrl_t *storm_ctrl)
{
    MW_ERROR_NO_T rc = MW_E_OK;

    storm_ctrl_t db_storm_ctrl = {0};
    rc = _mqttd_get_storm_control_cfg(port_idx, &db_storm_ctrl);
    if (MW_E_OK != rc)
    {
        mqttd_debug("Get %d storm ctrl cfg failed!\n", port_idx);
        return rc;
    }

    if (db_storm_ctrl.uc_cfg != storm_ctrl->uc_cfg)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_UC_CFG, port_idx, &storm_ctrl->uc_cfg, sizeof(storm_ctrl->uc_cfg));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set %d uc cfg failed\n", port_idx);
        }
    }

    if (db_storm_ctrl.mc_cfg != storm_ctrl->mc_cfg)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_MC_CFG, port_idx, &storm_ctrl->mc_cfg, sizeof(storm_ctrl->mc_cfg));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set mc cfg failed\n");
        }
    }

    if (db_storm_ctrl.bc_cfg != storm_ctrl->bc_cfg)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_BC_CFG, port_idx, &storm_ctrl->bc_cfg, sizeof(storm_ctrl->bc_cfg));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set bc cfg failed\n");
        }
    }
    if (db_storm_ctrl.bc_mode != storm_ctrl->bc_mode)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_BC_MODE, port_idx, &storm_ctrl->bc_mode, sizeof(storm_ctrl->bc_mode));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set bc mode failed\n");
        }
    }
    if (db_storm_ctrl.uc_mode != storm_ctrl->uc_mode)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_UC_MODE, port_idx, &storm_ctrl->uc_mode, sizeof(storm_ctrl->uc_mode));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set uc mode failed\n");
        }
    }
    if (db_storm_ctrl.mc_mode != storm_ctrl->mc_mode)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_MC_MODE, port_idx, &storm_ctrl->mc_mode, sizeof(storm_ctrl->mc_mode));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set mc mode failed\n");
        }
    }
    if (db_storm_ctrl.bc_rate != storm_ctrl->bc_rate)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_BC_RATE, port_idx, &storm_ctrl->bc_rate, sizeof(storm_ctrl->bc_rate));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set bc spd failed\n");
        }
    }
    if (db_storm_ctrl.uc_rate != storm_ctrl->uc_rate)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_UC_RATE, port_idx, &storm_ctrl->uc_rate, sizeof(storm_ctrl->uc_rate));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set uc spd failed\n");
        }
    }
    if (db_storm_ctrl.mc_rate != storm_ctrl->mc_rate)
    {
        rc = mqttd_queue_setData(M_UPDATE, PORT_CFG_INFO, PORT_STORM_MC_RATE, port_idx, &storm_ctrl->mc_rate, sizeof(storm_ctrl->mc_rate));
        if (MW_E_OK != rc)
        {
            mqttd_debug("Set mc spd failed\n");
        }
    }
...
static MW_ERROR_NO_T _mqttd_handle_setconfig_storm_control(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    // 访问解析后的数据
    cJSON *element = NULL;
    cJSON *item = NULL;
    uint8_t port_list[MW_DEFAULT_MAX_PORT_NUM];
    uint8_t port_num = 0;
    uint8_t i = 0;
    storm_ctrl_t storm_ctrl = {0};
    if (cJSON_IsArray(data_obj))
    {
        cJSON_ArrayForEach(item, data_obj)
        {
            ...
            cJSON *mc = cJSON_GetObjectItem(item, "mc");
            cJSON *uc = cJSON_GetObjectItem(item, "uc");

            if (cJSON_IsArray(p))
            {
                cJSON_ArrayForEach(element, p)
                {
                    port_list[port_num++] = element->valueint;
                }
            }
            if (cJSON_IsNumber(bc))
            {
                storm_ctrl.bc_cfg = STORM_CONTROL_CFG_ENABLE;
                storm_ctrl.bc_mode = STORM_CONTROL_MODE_KPS;
                storm_ctrl.bc_rate = bc->valueint * 1000;
            }
            if (cJSON_IsNumber(mc))
            {
                storm_ctrl.mc_cfg = STORM_CONTROL_CFG_ENABLE;
                storm_ctrl.mc_mode = STORM_CONTROL_MODE_KPS;
                storm_ctrl.mc_rate = mc->valueint * 1000;
            }
            if (cJSON_IsNumber(uc))
            {
                storm_ctrl.uc_cfg = STORM_CONTROL_CFG_ENABLE;
                storm_ctrl.uc_mode = STORM_CONTROL_MODE_KPS;
                storm_ctrl.uc_rate = uc->valueint * 1000;
            }
...
static MW_ERROR_NO_T _mqttd_handle_setconfig_data(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    ...
    cJSON_ArrayForEach(child, data_obj)
    {
        ...
        else if (osapi_strcmp(child->string, "loop_guard") == 0)
        {
            ...
            rc = _mqttd_handle_setconfig_loop_prevention(mqttdctl, child);
            if (MW_E_OK != rc)
            {
        ...
        else if (osapi_strcmp(child->string, "storm_control") == 0)
        {
            ...
            rc = _mqttd_handle_setconfig_storm_control(mqttdctl, child);
            if (MW_E_OK != rc)
            {
...
static MW_ERROR_NO_T _mqttd_handle_getconfig_loop_prevention(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    MW_ERROR_NO_T rc;
    DB_MSG_T *ptr_msg = NULL;
    UI8_T *ptr_data = NULL;
    ...
    cJSON_AddBoolToObject(data_obj, "loop_guard", ptr_lp->enable);
    MW_FREE(ptr_msg);
    return MW_E_OK;
}
...
static MW_ERROR_NO_T _mqttd_handle_getconfig_port_setting(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    ...
    for (i = 1; i <= PLAT_MAX_PORT_NUM; i++)
    {
        ...
        else if (port_cfg_info.admin_duplex == 0)
            cJSON_AddNumberToObject(json_port_entry, "du", 3);
        else
            cJSON_AddNumberToObject(json_port_entry, "du", 4);
...
// blank entry
        ...
        cJSON *json_port_mirror_entry = cJSON_CreateObject();
        cJSON_AddNumberToObject(json_port_mirror_entry, "gid", i + 1);
        cJSON *json_src_in_ports = cJSON_CreateArray();
        cJSON *json_src_dir = cJSON_CreateArray();
        for (j = 0; j < PLAT_MAX_PORT_NUM; j++)
        {
            ...
            {
                cJSON_AddItemToArray(json_src_in_ports, cJSON_CreateNumber(j + 1));
...
static MW_ERROR_NO_T _mqttd_handle_getconfig_poe_control(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    return MW_E_OK;
}
...
static MW_ERROR_NO_T _mqttd_handle_getconfig_total_vlan_setting(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    MW_ERROR_NO_T rc = MW_E_OK;
    DB_VLAN_ENTRY_T *ptr_vlan_entry_tbl = NULL;
    UI32_T *ptr_vlan_list_tbl = NULL;
...
static MW_ERROR_NO_T _mqttd_handle_getconfig_vlan_member(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    ...
    for (i = 1; i < MAX_VLAN_ENTRY_NUM + 1; i++)
    {
        ...
        if ((vlan_entry->vlan_id == i) && (vlan_entry->tagged_member == 0) && (vlan_entry->untagged_member == 0) && (vlan_entry->port_member == 0))
        {
            MW_FREE(ptr_msg);
            continue;
        }
...
static MW_ERROR_NO_T _mqttd_handle_getconfig_total_vlan_member(MQTTD_CTRL_T *mqttdctl, cJSON *data_obj)
{
    ...
    for (i = 0; i < MAX_VLAN_ENTRY_NUM; i++)
    {
        ...
        if ((ptr_vlan_entry_tbl->vlan_id[i] == i + 1) && (ptr_vlan_entry_tbl->tagged_member[i] == 0) && (ptr_vlan_entry_tbl->untagged_member[i] == 0) && (ptr_vlan_entry_tbl->port_member[i] == 0))
        {
            continue;
        }
        // 创建第一个对象并添加到数组
    ...
    cJSON_ArrayForEach(child, data_obj)
    {
        if (cJSON_IsString(child) && (child->valuestring != NULL))
        {
            ...
            else if (osapi_strcmp(child->valuestring, "loop_guard") == 0)
            {
                ...
                rc = _mqttd_handle_getconfig_loop_prevention(mqttdctl, data);
                if (MW_E_OK != rc)
                {
                    mqttd_debug("Handling setConfig device failed.");
                    break;
            ...
            {
                // Handle "poe_control" case
                rc = _mqttd_handle_getconfig_poe_control(mqttdctl, data);
                if (MW_E_OK != rc)
                {
                    mqttd_debug("Handling setConfig jumbo_frame failed.");
                    break;
...
MW_ERROR_NO_T _mqttd_deinit(void)
{
    ...
    if (NULL != ptr_mqttd_time)
    {
        mqttd_debug("MQTT free the timer.");
    ...
    {
        mqttd_debug("MQTT free the remain message semaphore.");
        osapi_mutexDelete(ptr_mqttmutex);
        ptr_mqttmutex = NULL;
    }
    /*free queue*/
